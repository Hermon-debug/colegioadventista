...
if (preRegistrationForm) {
  preRegistrationForm.addEventListener("submit", async (e) => {
    ...
    try {
      const response = await fetch("processar-pre-inscricao.php", {
        method: "POST",
        body: formData,
      })
      ...
      if (data.success) {
        ...
        document.querySelector(".pre-registration-section").scrollIntoView({
          behavior: "smooth",
          block: "start",
        })
      } else {
        alert(data.message || "Erro ao processar pré-inscrição.")
      }
    } catch (error) {
